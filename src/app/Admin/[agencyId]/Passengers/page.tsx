...
export default function PassengersPage() {
  ...
  const fetchPassengers = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/passengers/getAllPassengers?agencyId=${agencyId}`,
  ...
  const handleAddPassenger = async () => {
    ...
    try {
      ...
      if (editId !== null) {
        await axios.put(
          `${process.env.NEXT_PUBLIC_API_URL}/api/passengers/updatePassenger/${editId}`,
          { name, age, phone, agencyId },
          { headers: { Authorization: `Bearer ${freshToken}` } }
        );
      } else {
        await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/api/passengers/addPassenger`,
          { name, age, phone, agencyId },
          { headers: { Authorization: `Bearer ${freshToken}` } }
        );
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/passengers/deletePassenger/${id}`,
        {
          headers: { Authorization: `Bearer ${freshToken}` },
          data: { agencyId },
        }
