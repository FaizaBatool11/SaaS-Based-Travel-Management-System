...
//           { name: roleName, description, permissions: selectedPermissions, agencyId }, // 👈 added agencyId
//           { headers: { Authorization: `Bearer ${token}` } }
//         );
...
//       const token = localStorage.getItem("token");
//       await axios.delete(
//       `http://localhost:5000/api/roles/${roleId}?agencyId=${agencyId}`, // 👈 added
//       { headers: { Authorization: `Bearer ${token}` } }
//     );
...
export default function AddRolesPage() {
  ...
  const handleDeleteRole = async (roleId: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `http://localhost:5000/api/roles/${roleId}?agencyId=${agencyId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setRoles((prev) => prev.filter((r) => r.id !== roleId));
    } catch (err) {
