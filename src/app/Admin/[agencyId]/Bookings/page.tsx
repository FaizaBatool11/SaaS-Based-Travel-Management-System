...
//   // 🔹 Fetch trips, passengers, bookings after token is loaded
//   useEffect(() => {
...
//     try {
//       const res = await axios.patch(
//         `http://localhost:5000/api/bookings/cancelBooking/${id}/cancel`,
//         {},
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       setBookings(bookings.map((b) => (b.id === id ? res.data : b)));
...
//           headers: { Authorization: `Bearer ${token}` },
//         });
//         const tripsArray: Trip[] = Array.isArray(res.data) ? res.data : res.data.trips || [];
//         setTrips(tripsArray);
//       } catch (err) {
//         console.error(err);
//         setTrips([]);
...
//           headers: { Authorization: `Bearer ${token}` },
//         });
//         const passengersArray: Passenger[] = Array.isArray(res.data)
//           ? res.data
//           : res.data.passengers || [];
...
//           headers: { Authorization: `Bearer ${token}` },
//         });
//         const bookingsArray: Booking[] = Array.isArray(res.data)
//           ? res.data
//           : res.data.bookings || [];
...
//       const updatedBooking: Booking = res.data;
...
//                   <td className="p-3 border-b">{b.trip?.name || "N/A"}</td>
//                   <td className="p-3 border-b">{b.seatsAvailable}</td>
//                   <td className="p-3 border-b capitalize">{b.status}</td>
//                   <td className="p-3 border-b flex gap-2">
//                     {b.status === "pending" ? (
//                       <>
...
const updateStatus = async (id: number, status: string) => {
  try {
    ...
    const updatedBooking: Booking = res.data.booking; // backend se "booking" return ho rahi hai

    // state me sirf us booking ko update karo
    setBookings((prev) =>
            ...
            {filteredBookings.length > 0 ? (
              filteredBookings.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50">
                  <td className="p-3 border-b">{b.id}</td>
                  <td className="p-3 border-b">{b.passenger?.name || "N/A"}</td>
                  <td className="p-3 border-b">{b.trip ? `${b.trip.from} → ${b.trip.to}` : "N/A"}</td>
                  <td className="p-3 border-b">{b.seatsAvailable}</td>
                  <td className="p-3 border-b capitalize">{b.status}</td>
                  <td className="p-3 border-b flex gap-2">
                    {b.status === "pending" ? (
