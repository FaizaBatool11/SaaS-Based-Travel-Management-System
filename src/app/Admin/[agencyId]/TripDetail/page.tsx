...
//   const agencyId = params.agencyId as string; // still used for UI navigation
...
//       const activeAgencyId = localStorage.getItem("activeAgencyId") || agencyId;
//       if (!activeAgencyId) {
//         setError("No agency selected. Please switch agency.");
//         setTrips([]);
//         return;
//       }
...
//   fetchTrips();
// }, [agencyId]); // run when route param agencyId changes
...
//     setNewTrip((prev) => ({ ...prev, [name]: Number(value) }));
//     return;
//   }

//   // keep strings for others (depart stays string, datetime-local)
...
//     if (isEditing) {
//       // include agencyId in body to satisfy backend checks
...
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       // if backend returns updated trip in res.data.trip (or res.data), adapt accordingly:
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          ...
          {/* ✅ Add Trip button only if user has trips:create */}
          <PermissionGate required="trips:create">
            <button
              onClick={() => {
                setIsEditing(false);
                setShowModal(true);
            ...
            {filteredTrips.map((trip) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex gap-3">
                    {/* ✅ Edit only if trips:update */}
                    <PermissionGate required="trips:update">
                      <button
                        onClick={() => handleEdit(trip)}
                        className="p-2 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600"
                      >
                    ...
                    {/* ✅ Delete only if trips:delete */}
                    <PermissionGate required="trips:delete">
                      <button
                        onClick={() => handleDelete(trip.id)}
                        className="p-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-600"
                      >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <input
                  ...
                  value={newTrip.from}
                  onChange={handleChange}
                  className="p-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500"
                  required
                />
                <input
                  type="text"
